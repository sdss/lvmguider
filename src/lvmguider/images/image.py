...
class Image:
    ...
    def __init__(
        ...
        ----------
        data
            Numpy array containing data for image.
        ...
        catalog
            Catalog table.
        meta
            Dictionary with meta information (note: not preserved in I/O operations).
    ...
    def from_bytes(cls, data: bytes) -> Image:
        ...
        ----------
        data
            Bytes array to create image from.
        ...
        with io.BytesIO(data) as bio:
            # read whole file
            data = fits.open(bio, memmap=False, lazy_load_hdus=False)  # type: ignore
    ...
    def from_ccddata(cls, data: CCDData) -> Image:
        ...
        image = Image(
            ...
            else data.uncertainty.array.astype(np.float32),
        )
        return image
    ...
    def to_ccddata(self) -> CCDData:
        ...
        return CCDData(
            ...
            else StdDevUncertainty(self.uncertainty),
            unit="adu",
        )
    ...
    def set_meta(self, meta: Any) -> None:
        ...
        Note that it is possible to store, e.g., strings, but they would be stored as
        img.meta[str] and be overwritten with every new string, which is probably not
        what you want. Use the img.meta dict directly for this and set_meta/get_meta
        only for class-based data.

        Parameters
        ----------
        meta
            Meta information to store.
    ...
    def get_meta(self, meta_class: Type[MetaClass]) -> MetaClass:
        """Returns meta information, assuming that it is stored under the class
        of the object.

        Parameters
        ----------
        meta_class
            Class to return meta information for.
    ...
    def get_meta_safe(
        self,
        meta_class: Type[MetaClass],
        default: Optional[MetaClass] = None,
    ) -> Optional[MetaClass]:
        """Calls get_meta in a safe way and returns default value on exception."""
